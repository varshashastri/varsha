  ...
  for i in range(size.rows):
	...
	if textboxes[(i,j)].type==BoxType.Question:
		...
		while textboxes[(i-x,j)].type==BoxType.Question:
			x=x+1
		#print textboxes[(i,j-y)].AcrossValue,"and",textboxes[(i-x,j)].DownValue
		#print textboxes[(i,j-y)].acrosslist,"and",textboxes[(i-x,j)].downlist
		#print textboxes[(i-x,j)].acrosslist,textboxes[(i,j-y)].downlist
...
#	for j in range(size.columns):
#	   if textboxes[(i,j)].type==BoxType.Question:
#		 textboxes[(i,j)].possibleval=[1,2,3,4,5,6,7,8,9]
#	   else:
#		 textboxes[(i,j)].possibleval=[]
#  backtrack.track(textboxes)
#  for i in range(size.rows):
#     for j in range(size.columns):
